...
#define RECORD_MAX_SIZE    1600
    ...
    if(data_len==4){
        ...
        int row = 3;
        int key = -1;
        ALOGE("testAudioIN:len:%d  %02x %02x %02x \n",data_len, data[0],data[1],data[2]);
        if(loop_act==0x20){
            int key = -1;
            int input_devices=0;
            ...
            snprintf(write_buf,
                sizeof(write_buf) - 1,"audio_loop_test=1;test_in_stream_route=0x%x;test_stream_route=0x%x;samplerate=%d;channels=%d",
