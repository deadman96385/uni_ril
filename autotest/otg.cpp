...
int otg_is_support(void)
{
	...
	if(strncmp(is_support_otg, "1", sizeof("1")) == 0)
	{
		ret = 1;
...
int otgEnable(void)
{
	...
	if(otg_is_support() == 1)
	{
		...
		if(strncmp(host_enable, "enabled", sizeof("enabled")) == 0)
		{
			ret = 0;
		}
		else
...
int otgDisable(void)
{
	...
	if(otg_is_support() == 1)
	{
		...
		host_enable_fd = open(HOST_ENABLE_PATH, O_RDONLY);
		read(host_enable_fd, host_enable, 8);
		if(strncmp(host_enable, "disabled", sizeof("disabled")) == 0)
		{
			ret = 0;
		}
		else
...
int otgIdStatus(void)
{
	...
	if(otg_is_support() == 1)
	{
		...
		else
		{
			...
			if(strncmp(otg_status, "high", sizeof("high")) == 0)
			{
				ret = 1;
			}
			else if(strncmp(otg_status, "low", sizeof("low")) == 0)
			{
				ret = 0;
			}
			close(otg_status_fd);
