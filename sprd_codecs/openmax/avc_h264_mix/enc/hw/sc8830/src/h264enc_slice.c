                                      ...
                                      ENC_PPS_T *pps, int32 i_type, int32 i_idr_pic_id, int32 i_frame, int32 i_qp)
{
    ...
    sh->i_type = i_type;
    //sh->i_first_mb = 0;
    sh->i_last_mb = IClip(0, (img_ptr->frame_size_in_mbs-1), (sh->i_first_mb+img_ptr->slice_mb-1));
...
PUBLIC int32 H264Enc_slice_write (H264EncObject *vo, ENC_IMAGE_PARAMS_T *img_ptr)
{
    ...
    if (VSP_READ_REG_POLL(GLB_REG_BASE_ADDR + VSP_INT_RAW_OFF, V_BIT_2, V_BIT_2, TIME_OUT_CLK,  "Polling MBW_FMR_DONE")) //check MBW is done
    {
        return 0;
    }
