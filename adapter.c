...
const struct cmd_table single_at_cmd_cvt_table[] = {
    ...
    {AT_CMD_UNKNOWN, AT_CMD_TYPE_GEN, AT_CMD_STR("AT"), cvt_generic_cmd_req, 50},
    //  NULL,
};
...
int phoneserver_deliver_at_cmd(const pty_t * pty, char *cmd, int len)
{
    ...
    if (multiSimMode == 1) {
        ...
        for (i = 0; i < count; i++) {
            if (!strncasecmp(multi_at_cmd_cvt_table[i].cmd_str, cmd,
...
int adapter_cmux_write(cmux_t * mux, char *buf, int __attribute__((unused)) len, int to) {
    int ret, res;
    pid_t tid;
    tid = gettid();
    int seconds;
...
int adapter_cmux_write_for_ps(cmux_t * mux, char *buf,
    ...
    seconds = time((time_t *) NULL);
    PHS_LOGD("[%d] before timeout (%d)\n", tid, seconds);
    int err = thread_cond_timedwait(&mux->cond_timeout, &mux->mutex_timeout, &timeout);
    if (err == ETIMEDOUT) {
        mux->cp_blked += 1;
        PHS_LOGE("mux %s command %s time out", mux->name, str);
        if (mux->cp_blked > BLOCKED_MAX_COUNT) {
            ...
            if (soc_client > 0) {
                res = write(soc_client, block_str, strlen(block_str) + 1);
                ALOGD("write %d bytes to soc_client:%d to info modem is blocked", res, soc_client);
            }
        }
        seconds = time((time_t *) NULL);
        PHS_LOGD("[%d] timeout (%d)\n", tid, seconds);
    ...
    if (findInBuf(input, len, "+XX")) {
        ...
        if (epin_rsp.rsp_index == 4) {
            snprintf(rsp_str, sizeof(rsp_str), "+EPIN:%d,%d,%d,%d\r",
                    epin_rsp.rsp[0], epin_rsp.rsp[1], epin_rsp.rsp[2], epin_rsp.rsp[3]);
            adapter_pty_write(epin_rsp.pty, rsp_str, strlen(rsp_str));
        }
    } else if (adapter_cmd_is_end(rsp->rsp_str, rsp->len) == TRUE) {
        adapter_cmux_deregister_callback(rsp->recv_cmux);
...
int cvt_cmgs_cmgw_test_req(AT_CMD_REQ_T * req) {
    ...
    mux = adapter_get_cmux(req->cmd_type, TRUE);
    adapter_cmux_register_callback(mux, cvt_cmgs_cmgw_test_rsp, (unsigned long) req->recv_pty);
    adapter_cmux_write(mux, req->cmd_str, req->len, req->timeout);
    return AT_RESULT_PROGRESS;
}
...
int cvt_cmgs_cmgw_edit_callback(pty_t * pty, char *cmd, int len,
    ...
    adapter_cmux_register_callback(mux, cvt_cmgs_cmgw_set_rsp, (unsigned long) pty);
...
int cvt_cmgs_cmgw_set_req(AT_CMD_REQ_T * req) {
    ...
    if (req->recv_pty->cmgs_cmgw_set_result == 1) {
        adapter_pty_enter_editmode(req->recv_pty, (void *) cvt_cmgs_cmgw_edit_callback, (unsigned long) mux);
...
int cvt_cmgs_cmgw_set_rsp2(AT_CMD_RSP_T * rsp, unsigned long user_data) {
    ...
    if (findInBuf(rsp->rsp_str, rsp->len, "NO CARRIER")) {
        if (multiSimMode == 1) {
            ind_pty = adapter_get_ind_pty((mux_type) (rsp->recv_cmux->type));
            ind_eng_pty = adapter_multi_get_eng_ind_pty(
                    (mux_type) (rsp->recv_cmux->type));
        } else {
            ind_pty = adapter_get_default_ind_pty();
            ind_eng_pty = adapter_single_get_eng_ind_pty();
        }
...
int cvt_esatenvecmd_set_req(AT_CMD_REQ_T * req) {
    ...
    do {
        ...
        snprintf(at_cmd_str, sizeof(at_cmd_str), "AT+ESATENVECMD=\"%s\"\r",
                data);
        mux = adapter_get_cmux(req->cmd_type, TRUE);
        adapter_cmux_register_callback(mux, cvt_generic_cmd_rsp, (unsigned long) req->recv_pty);
        adapter_cmux_write(mux, at_cmd_str, strlen(at_cmd_str), req->timeout);
...
int cvt_echupvt_set_req(AT_CMD_REQ_T * req) {
    ...
    adapter_cmux_register_callback(mux, cvt_echupvt_set_rsp, (unsigned long) req->recv_pty);
...
int cvt_ath_cmd_req(AT_CMD_REQ_T * req) {
    ...
    if (req->recv_pty->type != STMAT) {
        mux = adapter_get_cmux(req->cmd_type, TRUE);
        adapter_cmux_register_callback(mux, cvt_ath_cmd_rsp, (unsigned long) req->recv_pty);
        adapter_cmux_write(mux, req->cmd_str, req->len, req->timeout);
    } else {
...
int cvt_snvm_set_req(AT_CMD_REQ_T * req) {
    ...
    if (req->recv_pty->cmgs_cmgw_set_result == 1) {
        adapter_pty_enter_editmode(req->recv_pty,
                (void *) cvt_snvm_edit_callback, (unsigned long) mux);
        adapter_pty_write(req->recv_pty, "> ", strlen("> "));
        return AT_RESULT_PROGRESS;
    } else if (req->recv_pty->cmgs_cmgw_set_result == 2) {
        adapter_pty_end_cmd(req->recv_pty);
