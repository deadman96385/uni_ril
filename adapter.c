...
int rxlev[SIM_COUNT], ber[SIM_COUNT], rscp[SIM_COUNT], ecno[SIM_COUNT], rsrq[SIM_COUNT], rsrp[SIM_COUNT];
...
int phoneserver_deliver_indicate_default(const cmux_t *cmux, char *cmd, int len) {
    int ret;
...
int phoneserver_deliver_at_cmd(const pty_t *pty, char *cmd, int len) {
    ...
    i = 0;
    if (SIM_COUNT != 1) {
        count = NUM_ELEMS(multi_at_cmd_cvt_table);
        for (i = 0; i < count; i++) {
        ...
        count = NUM_ELEMS(single_at_cmd_cvt_table);
        for (i = 0; i < count; i++) {
            if (!strncasecmp(single_at_cmd_cvt_table[i].cmd_str, cmd,
    ...
    if (SIM_COUNT != 1) {
        ...
        default:
            PHS_LOGE(
                    "AT command is written to the error PTY channel! \
                    pty->type=%d\n", pty->type);
        }
    } else {
...
int adapter_cmux_write(cmux_t *mux, char *buf, int __attribute__((unused)) len,
    ...
    if (err == ETIMEDOUT) {
        ...
        if (mux->cp_blked > BLOCKED_MAX_COUNT) {
            mux->cp_blked = 0;
            strcpy(block_str, "Modem Blocked");
            if (soc_client < 0) {
...
int cvt_cmgs_cmgw_set_req(AT_CMD_REQ_T *req) {
    ...
    if (req->recv_pty->cmgs_cmgw_set_result == 1) {
        adapter_pty_enter_editmode(req->recv_pty,
                (void *)cvt_cmgs_cmgw_edit_callback, (unsigned long)mux);
