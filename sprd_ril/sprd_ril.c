    ...
    if (strStartsWith(s, "+CTZV:")) {
        ...
        if (strStartsWith(s, "+CREG:")) {
            ...
            if (commas >= 3) {
                ...
                if (err < 0)
                    goto out;
                snprintf(cmd, sizeof(cmd), "ril_net=%d;", net_type);
                RILLOGD("Send Audio Test Cmd, net type is %d, cmd is \"%s\" .",
                        net_type, cmd);
                SendAudioTestCmd(cmd, sizeof(cmd));
            }
        }
    ...
    if (fd < 0) {
        ...
        do {
            ...
            if (ret > 0) {
                ...
                RILLOGE("pipe write error %d, bytes read is %d", errno, bytes_to_read - bytes);
                break;
            } else {
                RILLOGD("pipe_write_warning: %d, ret is %d", errno, ret);
            }
        } while (bytes);
    }
