...
int sendTRData(int socket_id, char *data) {
    ...
    int sendTRChannelID = getChannel();
    ...
    RLOGD("sendTRData cmd:%s", cmd);
    err = at_send_command_singleline(ATch_type[sendTRChannelID], cmd,
                                     "+SPUSATTERMINAL:", &p_response);
    RLOGD("at_send_command_singleline exit");
    free(cmd);
...
int sendELData(int socket_id, char *data) {
    ...
    int err;
    ATResponse *p_response = NULL;
    int sendELChannelID = getChannel();
    ...
    err = at_send_command_singleline(ATch_type[sendELChannelID], cmd,
...
void sendEventLoop(CatResponseMessageSprd *pResMsg, int event, int socket_id) {
    ...
    convertBinToHex(buf,bufCount,hexString);
...
static void *openChannelThread(void *param) {
    ...
    if (pstkContext->pOCData->DataDstAddressType != ADDRESS_TYPE_IPV4 && pstkContext->pOCData->DataDstAddressType != ADDRESS_TYPE_IPV6) {
    ...
    while (writeOffset < len) {
        ...
        do {
            written = write(fd, toWrite + writeOffset,
                                len - writeOffset);
            RLOGD("send to:%d %d byte", fd, (int)(len - writeOffset));
