...
extern void RIL_register_ATCIServer(
        RIL_RadioFunctions *(*Init)(const struct RIL_Env *, int, char **),
...
OpenLib:
    ...
    rilATCIInit = (const RIL_RadioFunctions *(*)(const struct RIL_Env *, int, char **))dlsym(dlHandle, "RIL_ATCI_Init");
