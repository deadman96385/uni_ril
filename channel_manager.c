...
static int get_pty(master_fdp, slave_fdp, slave_name, uid)
    int *master_fdp;int *slave_fdp;char *slave_name;int uid; {
    int i, mfd, sfd = -1;
    char pty_name[20];
    struct termios tios;
    ...
    if (mfd >= 0) {
        ...
        if (rett >= 0) {
            ...
            if (chmod(pty_name, S_IRUSR | S_IWUSR | S_IRGRP | S_IWGRP) < 0) {
                PHS_LOGE("CHNMNG: Couldn't change pty slave %s's mode", pty_name);
            }
#ifdef TIOCSPTLCK
            ptn = 0;
            if ((rett = ioctl(mfd, TIOCSPTLCK, &ptn)) < 0) {
                PHS_LOGE("CHNMNG: Couldn't unlock pty slave %s rett=%x", pty_name, rett);
            }
#endif
            if ((sfd = open(pty_name, O_RDWR | O_NOCTTY | O_NONBLOCK)) < 0) {
                PHS_LOGE("CHNMNG: Couldn't open pty slave %s sfd=%x", pty_name, sfd);
            }
        } else {
...
static cmux_t *chnmng_get_cmux(void * const chnmng, const AT_CMD_TYPE_T type,
    ...
    PHS_LOGI("Send thread TID [%d] CHNMNG enter channel_manager_get_cmux ", tid);
    pty = find_pty(me, tid);
    int index = 0;
    ...
    while (mux == NULL && block) {
        ...
        if (mux)
            break;
        PHS_LOGI("Send thread TID [%d] CHNMNG block at channel_manager_get_cmux ", tid);
    ...
    if (mux) {
        mux->pty = pty;
        mux->cmd_type = type;
        PHS_LOGI("Send thread TID [%d] CHNMNG Leave channel_manager_get_cmux:%s ", tid, mux->name);
    }
    return mux;
}
        ...
        for (sim_index = 0; sim_index < simNum; sim_index++) {
                ...
                if (rscp_array[N - 1] >= rscp_array[N - 2]) { //signal go up
                    ...
                    if (rscp_array[N - 1] == rscp_array[N - 2] - 1) {
                        ...
                        if (rscp_value < 0 || rscp_value > 31
                                || rscp_value < rscp[sim_index]) { //if invalid, use current value
                            rscp_value = rscp[sim_index];
                        }
                ...
                if (rxlev_array[N - 1] >= rxlev_array[N - 2]) { //signal go up
                    ...
                    if (rxlev_array[N - 1] == rxlev_array[N - 2] - 1) {
                        ...
                        if (rxlev_value < 0 || rxlev_value > 31
                                || rxlev_value < rxlev[sim_index]) { //if invalid, use current value
                            rxlev_value = rxlev[sim_index];
                        }
            ...
            if (rscp_array != NULL) // l/tl/lf
                snprintf(ind_str, sizeof(ind_str), "\r\n+CESQ: %d,%d,%d,%d,%d,%d\r\n", rxlev_value,
                        ber[sim_index], rscp_value, ecno[sim_index], rsrq[sim_index], rsrp_value);
            else
                // w/t
                snprintf(ind_str, sizeof(ind_str), "\r\n+CSQ: %d,%d\r\n",
...
static void *detect_at_no_response(void __attribute__((unused)) *par) {
    ...
    soc_fd = socket_local_server(socket_name, ANDROID_SOCKET_NAMESPACE_ABSTRACT,
            SOCK_STREAM);
    if (soc_fd < 0) {
