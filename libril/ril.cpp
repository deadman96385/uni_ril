...
static void listenCallback_EXT(int fd, short flags, void *param);
...
// TODO: since only primary card connected at one time, one writeMutex is OK
...
static int processCommandBuffer(void *buffer, size_t buflen,
    ...
    if (request == RIL_REQUEST_OEM_HOOK_STRINGS && socket_type == RIL_ATCI_SOCKET) {
        ...
        if (status != NO_ERROR || simId < RIL_SOCKET_1
                ...
                RLOGE("invalid simId");
                return 0;  // TODO: need debug should not return 0;
        }
...
static int responseDataCallList(Parcel &p, void *response, size_t responselen) {
    if (s_callbacks.version < 5) {
        ...
        for (i = 0; i < num; i++) {
            ...
            appendPrintBuf("%s[status=%d,retry=%d,cid=%d,%s,%s,%s,%s,%s,%s,%s,mtu=%d],",
                printBuf,
                p_cur[i].status,
                p_cur[i].suggestedRetryTime,
                p_cur[i].cid,
...
static int responseRilSignalStrength(Parcel &p, void *response,
                                      size_t responselen) {
    if (response == NULL && responselen != 0) {
        RLOGE("invalid response: NULL");
        return RIL_ERRNO_INVALID_RESPONSE;
...
static int responseCellInfoList(Parcel &p, void *response, size_t responselen) {
    ...
    int i;
    for (i = 0; i < num; i++) {
        appendPrintBuf("%s[%d: type=%d,registered=%d,timeStampType=%d,timeStamp=%lld",
                printBuf, i, p_cur->cellInfoType, p_cur->registered,
                p_cur->timeStampType, p_cur->timeStamp);
...
static int responseActivityData(Parcel &p, void *response, size_t responselen) {
    ...
    startResponse;
    appendPrintBuf("Modem activity info received: sleep_mode_time_ms %d \
          idle_mode_time_ms %d tx_mode_time_ms %d %d %d %d %d and rx_mode_time_ms %d",
    ...
    while (1) {
        ...
        while ((call_list->next != call_list) || (sim_list->next != sim_list)) {
            if (call_list->next != call_list) {  // call_list has priority
                ...
                RLOGI("-->Normal Dispatch call list", tid);
            } else {
                cmd_item = sim_list->next;
                RLOGI("-->Normal Dispatch sim list", tid);
            }
            processCommandBuffer(cmd_item->p_reqData->buffer,
                cmd_item->p_reqData->buflen, socket_id, cmd_item->p_reqData->socket_type);
    ...
    for (;;) {
        ...
        if (ret == 0 && p_record == NULL) {
            ...
            if (pCI->requestNumber == RIL_REQUEST_SEND_SMS
                ...
                RLOGD("Add '%s' to simReqList\n", requestToString(pCI->requestNumber));
                list_add_tail(cmdList->simReqList, cmd_item, socket_id);
                pthread_mutex_lock(&(cmdList->normalDispatchMutex));
                pthread_cond_signal(&(cmdList->normalDispatchCond));
                pthread_mutex_unlock(&(cmdList->normalDispatchMutex));
            } else {
                RLOGD("Add '%s' to otherReqList\n", requestToString(pCI->requestNumber));
...
static void startListenIMS(RIL_SOCKET_ID socket_id, SocketListenParam *socket_listen_p) {
    int fdListen = -1;
    int ret;
    char socket_name[20];
    ...
    RLOGI("Start to listen %s", socket_name);
    ...
    /* note: non-persistent so we can accept only one connection at a time */
    ril_event_set(socket_listen_p->listen_event, fdListen, false,
                listenCallback_EXT, socket_listen_p);
...
extern "C" void RIL_register(const RIL_RadioFunctions *callbacks) {
    ...
    for (int i = RIL_REQUEST_LAST + 1; i <= RIL_REQUEST_LAST +
               RIL_IMS_REQUEST_LAST - RIL_IMS_REQUEST_BASE; i++) {
        assert(i - RIL_REQUEST_LAST ==
                        s_commands[i].requestNumber - RIL_IMS_REQUEST_BASE);
    }

    int count = RIL_UNSOL_RESPONSE_LAST - RIL_UNSOL_RESPONSE_BASE;
    for (int i = count + 1; i <= count + RIL_IMS_UNSOL_RESPONSE_LAST
              - RIL_IMS_UNSOL_RESPONSE_BASE + 1; i++) {
    ...
    if (ret < 0) {
        RLOGE("Failed to listen on control socket '%d': %s",
                fdListen, strerror(errno));
        exit(-1);  // TODO: is exit or not
    }
    s_atciSocketParam.fdListen = fdListen;

    ril_event_set(&s_atciListenEvent, fdListen, true,
            listenCallback_EXT, &s_atciSocketParam);
    ...
    for (simId = 0; simId < SIM_COUNT; simId++) {
        startListenIMS((RIL_SOCKET_ID)(RIL_SOCKET_1 + simId), &s_imsSocketParam[simId]);
    }
}
...
static int checkAndDequeueRequestInfo(RequestInfo *pRI) {
    int ret = 0;
...
#endif
{
    ...
    if (unsolResponse <= RIL_UNSOL_RESPONSE_LAST) {
        unsolResponseIndex = unsolResponse - RIL_UNSOL_RESPONSE_BASE;
        ...
        unsolResponseIndex = unsolResponse - RIL_IMS_UNSOL_RESPONSE_BASE
                + RIL_UNSOL_RESPONSE_LAST - RIL_UNSOL_RESPONSE_BASE + 1;
...
// TODO: is or not could multiplex with listencallback
static void listenCallback_EXT(int fd, short flags __unused, void *param) {
    int ret;
    int fdCommand;
    RecordStream *p_rs;
    bool persist = false;
    ...
    RLOGI("libril: new connection to %s",
            (p_info->type == RIL_ATCI_SOCKET)? "ATCI" : "IMS");
