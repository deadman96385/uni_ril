...
Return<void> SecureElementImpl::openLogicalChannel(const hidl_vec<uint8_t>& aid, uint8_t p2, openLogicalChannel_cb _hidl_cb) {
#if VDBG
    ...
    //SE_LogicalChannelResponse *result = NULL;
    LogicalChannelResponse response = {};

    if (aidLen == 0) {
        RLOGE("SE openLogicalChannel aid length is zero");
        stat = NO_SUCH_ELEMENT_ERROR;
        goto done;
    }
    ...
    if (numInts < 1) {
        RLOGD("SE openLogicalChannel failed");
        goto done;
    }
...
void secureElement::registerService(SE_Functions *seCallbacks) {
    ...
    for (int i = 0; i < simCount; i++) {
        ...
        secureElementService[i]->mSlotId = i;
