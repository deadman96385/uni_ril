...
threadpool_t *
thread_pool_init(int    max_threads,
                 int    idle_timeout) {
    ...
    if (pthread_attr_setdetachstate(&(threadpool->pool_attr),
...
void
thread_pool_destroy(threadpool_t *threadpool) {
...
thread_pool_worker(void *data) {
    ...
    for (;;) {
        ...
        if (worker) {
            worker->handler(worker->data, worker->socket_id);
            free(worker);
            worker = NULL;
        } else if (must_exit) {
            break;
...
int
thread_pool_dispatch(threadpool_t *     threadpool,
    ...
    new_worker->socket_id = socket_id;
