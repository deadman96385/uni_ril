...
struct exfat_dev* exfat_open(const char* spec, enum exfat_mode mode)
{
	...
	if (dev == NULL)
	{
		printf("failed to allocate memory for device structure");
		return NULL;
	}
	...
	case EXFAT_MODE_RO:
		...
		{
			free(dev);
			printf("failed to open `%s' in read-only mode", spec);
			return NULL;
		}
	...
	case EXFAT_MODE_RW:
		...
		{
			free(dev);
			printf("failed to open `%s' in read-write mode", spec);
			return NULL;
		}
		...
		!S_ISREG(stbuf.st_mode))
	{
		close(dev->fd);
		free(dev);
		printf("`%s' is neither a device, nor a regular file", spec);
		return NULL;
	}
	...
	if (!S_ISREG(stbuf.st_mode))
	{
		...
		if (ioctl(dev->fd, DKIOCGETBLOCKCOUNT, &blocks) != 0)
		{
			close(dev->fd);
			free(dev);
			printf("failed to get blocks count");
			return NULL;
		}
	...
	if (!S_ISREG(stbuf.st_mode))
	{
		...
		if (ioctl(dev->fd, HDIO_GETGEO, &geom)) {
			close(dev->fd);
			free(dev);
			printf("Error getting  geom (err: %s) for '%s'. ", strerror(errno), spec);
			return NULL;
		}
		dev->start_sec = geom.start;

		if (exfat_seek(dev, 0, SEEK_SET) ==  (loff_t) -1)
		{
			close(dev->fd);
			free(dev);
			printf("failed to seek to the beginning of '%s'", spec);
			return NULL;
		}
	}
	else
#endif
	{
		...
		if (dev->size <= 0)
		{
			close(dev->fd);
			free(dev);
			printf("failed to get size of `%s'", spec);
			return NULL;
		}
	...
	if (dev->ufh == NULL)
	{
		close(dev->fd);
		free(dev);
		printf("failed to initialize ublio");
		return NULL;
	}
#endif
