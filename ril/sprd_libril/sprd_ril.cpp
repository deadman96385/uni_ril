...
blockingWrite(int fd, const void *buffer, size_t len) {
    ...
    while (writeOffset < len) {
        ...
        if (written >= 0) {
            ...
            RILLOGE ("RIL Response: unexpected error on write errno:%d, %s", errno, strerror(errno));
            close(fd);
            return -1;
        }
    }
...
slowDispatch(void *param) {
    ...
    while(1) {
        pthread_mutex_lock(&s_dispatchMutex);
        if ((&slow_cmd_list)->next == &slow_cmd_list) {
...
static void * normalDispatch(void *param) {
    ...
    while(1) {
        pthread_mutex_lock(&s_normalDispatchMutex);
        if (((&call_cmd_list)->next == &call_cmd_list) && ((&sim_cmd_list)->next == &sim_cmd_list)) {
...
RIL_register (const RIL_RadioFunctions *callbacks, int argc, char ** argv) {
    ...
    if (ret < 0) {
        RILLOGE("Failed to create slow dispatch thread errno:%d", errno);
        return;
    }
...
*/
    switch(request) {
        ...
        case RIL_REQUEST_SET_FACILITY_LOCK_FOR_USER: return "SET_FACILITY_LOCK_FOR_USER";//SPRD add for sim lock
        ...
        /* @} */
        case RIL_REQUEST_GET_IMS_SMSC_ADDRESS: return "GET_IMS_SMSC_ADDRESS";
