...
static void deactivateDataConnection(int channelID, void *data, size_t datalen, RIL_Token t)
{
    ...
    if (!IsLte) {
        ...
        RILLOGD("Try to deactivated is_stk_end_connectivity=%d, secondary_cid= %d ", is_stk_end_connectivity,secondary_cid);
        if (in4G) {
...
//SPRD add for simlock infors begin
static void requestGetSimLockInfors(int channelID, void *data, size_t datalen, RIL_Token t)
{
    ATResponse *p_response = NULL;
    int response[3] = {0};
    int err,skip,status,retrial;
    char *cmd, *line;
    int ret = -1;

    int fac = ((int*)data)[0];
    int ck_type = ((int*)data)[1];
    RILLOGD(" datalen = %d data[0] = %d data[1] = %d", (datalen/sizeof(int *)), fac, ck_type);
    ...
    RILLOGD("requestGetSimLockInfors: %s", cmd);
    ...
    err = at_tok_nextint(&line, &retrial);
    if (err < 0) goto error1;

    response[0] = fac;
    response[1] = status;
    response[2] = skip - retrial;
    RILLOGD("requestGetSimLockInfors status = %d retrial = %d remain times = %d", status, retrial, response[2] );
...
static int activeSpecifiedCidProcess(int channelID, void *data, int primaryCid, int secondaryCid, char* pdp_type) {
    ...
    if (errorHandlingForCGDATA(channelID, p_response, err, cid_index)) {
        ...
        snprintf(prop, sizeof(prop), "ro.modem.%s.eth", s_modem);
        property_get(prop, eth, "veth");
        snprintf(cmd, sizeof(cmd), "net.%s%d.ip_type", eth, cid_index);
        property_get(cmd, prop, "0");
        fb_ip_type = atoi(prop);
        RILLOGD( "Fallback: prop %s, fb_ip_type = %d", prop, fb_ip_type);
        if ( !(fb_ip_type == IPV4 && !strcmp(pdp_type,"IP")
                || (fb_ip_type == IPV6 && !strcmp(pdp_type,"IPV6")) ) ) {
            RILLOGD("Fallback pdp type mismatch, do deactive");
...
static char* checkNeedFallBack(int channelID, char *pdp_type,int cidIndex) {
    ...
    char prop[PROPERTY_VALUE_MAX] = {0};
    char eth[PROPERTY_VALUE_MAX]  = {0};
    char is_tecel_version[PROPERTY_VALUE_MAX] = {0};

    /* Check if need fall back or not */
    property_get(PERSIST_CURRENT_CARRIER, is_tecel_version, "default");
    RILLOGD("SetupDataCall is_tecel_version = %s", is_tecel_version);
    /* telcel can't fallback on cid 1 */
    if (strcmp(is_tecel_version, "telcel") == 0 && cidIndex == 0) {
        return ret;
    }
            ...
            for (i = 0; i < MAX_PDP_CP; i++) {
                ...
                if ( cid == (i + 1)) {
                    ...
                    if (i < MAX_PDP
                        ...
                        updatePDPCid(i+1,1);
                        /* check fall back or not */
                        fbIPType = checkNeedFallBack(channelID, pdp_type, i);
                        if(fbIPType != NULL) {
                            index = getPDP();
                            RILLOGD("FallBack cid index %d", index);
                            if(index < 0 || getPDPCid(index) >= 0) {
                                RILLOGD("Invalid cid index %d", index);
                            }else {
...
static void requestSendAT(int channelID, char *data, size_t datalen, RIL_Token t)
{
    ...
    } else if (!strncasecmp(at_cmd, "AT+FBCID", strlen("AT+FBCID"))) {
        // for get fallback's primary cid
        int cid =0;
        char* ptr=NULL;
        ptr = strchr(at_cmd, '=');
        if(ptr != NULL)
...
static void requestTransmitApduBasic(int channelID, void *data, size_t datalen, RIL_Token t){
    int err;
    int len;
    ATResponse *p_response = NULL;
    char cmd[128] = {0};
    ...
    RILLOGD("requestTransmitApdu INSTRUCTION = %02X", apdu->instruction);
    RILLOGD("requestTransmitApdu P1 = %02X", apdu->p1);
    RILLOGD("requestTransmitApdu P2 = %02X", apdu->p2);
    RILLOGD("requestTransmitApdu P3 = %02X", apdu->p3);
...
//            requestSendAT(channelID,data, datalen, t);
        ...
        break;
        case RIL_REQUEST_GET_HD_VOICE_STATE:
        {
            p_response = NULL;
            int response = 0;
            int mode = 0;
            ...
            if (err >= 0 && p_response->success) {
                ...
                skipNextComma(&line);
                err = at_tok_nextint(&line, &response);
                RILLOGD("RIL_REQUEST_GET_HD_VOICE_STATE:%d",response);
                if(err >= 0){
                    RIL_onRequestComplete(t, RIL_E_SUCCESS, &response,
                            sizeof(response));
                } else {
...
static void onUnsolicited (const char *s, const char *sms_pdu)
{
    ...
    /* SPRD: Add here for orange requirement */
        ...
        RILLOGD("RIL_UNSOL_STK_CC_ALPHA_NOTIFY");
        response = (RIL_StkCallControlResult *)alloca(sizeof(RIL_StkCallControlResult));
        if (response == NULL) goto out;
        line = strdup(s);
        tmp = line;
