    ...
    for (;;) {
        ...
        for (channelID = firstChannel; channelID < lastChannel; channelID++) {
            ...
            if (p_channelInfo[channelID].state == CHANNEL_IDLE) {
                RLOGD("channel%d state: '%d' \n",
                      p_channelInfo[channelID].channelID,
                      p_channelInfo[channelID].state);
...
static void pollSIMState(void *param) {
    ...
    switch (getSIMStatus(channelID)) {
        ...
        case SIM_NOT_READY: {
            RIL_requestTimedCallback(pollSIMState,
...
static void initializeCallback(void *param) {
    ...
    at_send_command(s_ATChannels[channelID], "at+spcapability=32,1,0", NULL);
    /* @} */
...
static void *mainLoop(void *param) {
 ...
 again:
        for (channelID = firstChannel; channelID < lastChannel; channelID++) {
            ...
            sprintf(p_channelInfo[channelID].ttyName, "/dev/CHNPTY%d", channelID);

            /* open TTY device, and attach it to channel */
            p_channelInfo[channelID].channelID = channelID;
            sprintf(p_channelInfo[channelID].name, "Channel%d", channelID);
...
void getProperty(RIL_SOCKET_ID socket_id, const char *property, char *value,
    ...
    int simId = 0;
    char prop[PROPERTY_VALUE_MAX];
    int len = property_get(property, prop, NULL);
    char *p[RIL_SOCKET_NUM];
    char *buf = prop;
    char *ptr = NULL;
    RLOGD("get [%s] property: %s", property, prop);
    if (strlen(prop) > 0 && len > 0) {
    ...
    memcpy(value, defaultVal, strlen(defaultVal) + 1);
    return;
}
